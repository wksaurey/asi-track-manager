...
class StampActualAPITest(TestCase):
    ...
    def setUp(self):
        ...
        now = localtime(timezone.now()).replace(hour=10, minute=0, second=0, microsecond=0)
        self.event = Event.objects.create(
    ...
    def test_stamp_with_custom_iso_time(self):
        self.client.login(username='stampadmin', password='Testpass123!')
        custom = self.event.start_time - timedelta(minutes=30)
        resp = self._post_stamp('start', time=custom.isoformat())
        self.assertEqual(resp.status_code, 200, resp.json() if resp.status_code != 200 else "")
        self.event.refresh_from_db()
